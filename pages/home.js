...
const Home = () => {
    ...
    const GROQ_API_KEY = 'aa-nRb5JgjAI4CbYyt1LL105fx10ZVwoLQyYeiwK4x4tcmcbKpm';
    ...
    // Use effect to scroll when streaming message changes
    useEffect(() => {
        scrollToBottom();
    }, [streamingMessage]);
    ...
    const processMessageWithLLM = async (messages, imageBase64 = null) => {
        try {
            setIsLoading(true);
            ...
            const formattedMessages = messages.map(msg => ({
                role: msg.role,
                content: msg.image
                    ? `${msg.content}\n[Attached image: ${msg.image.substring(0, 100)}...]` // Truncate image data
                    : msg.content
            }));
    ...
    const createNewChat = () => {
        const newChat = {
            id: Date.now(),
            messages: [],
            title: 'New Chat', // Set initial title to 'New Chat'
            lastUpdate: Date.now(),
            category: 'General'
        };
    ...
    const handleImageUpload = async (e) => {
        ...
        if (file) {
            try {
                ...
                // Auto-submit if there's also text in the prompt
                if (prompt.trim()) {
    ...
    const handleSubmit = async (e, imageBase64 = null) => {
        if (e) e.preventDefault();
        if ((!prompt.trim() && !imageBase64 && !selectedImage) || isLoading) return;
        ...
        try {
            const assistantMessage = await processMessageWithLLM(
                updatedMessages,
                userMessage.image
            );
            ...
            if (!currentChat) {
                // Always use the first text prompt for chat title, regardless of whether it's an image message
                let chatTitle = 'New Chat'; // Default title

                if (prompt.trim()) {
                    // If there's text in the prompt, use it for the title
                    chatTitle = prompt.length > 25
                        ? prompt.slice(0, 25) + '...'
                        : prompt;
                }
                ...
                // If it's the first message in an existing "New Chat" and it contains text
                if (currentChat.title === 'New Chat' && prompt.trim() && currentChat.messages.length === 0) {
                    const chatTitle = prompt.length > 25
                        ? prompt.slice(0, 25) + '...'
                        : prompt;
    ...
    const handleMessageEdit = async (messageId, newContent) => {
        ...
        try {
            ...
            // Only regenerate the immediate next message if it's an assistant message
